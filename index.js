...
function processSum(numberList, callback) {
  let sum = 0;
  numberList.length > 0
      ? (sum = numberList.reduce((num, num1) => num + num1))
      : sum;
  return callback(sum);
}
/**
...
function processContains(item, list, callback) {
  //Commented out stuff is things I tried to make work before looking
  //at the solutions code. I still tried to make it work but it does not.
...
function getRunnersByTShirtSize(runners, ShirtSize) {
  let runnersByTShirtSize = runners.filter(
    runners => runners.shirt_size === ShirtSize
...
function tallyUpDonations(runners) {
  const allDonations = runners.reduce ((num, runners) => num + runners.donation, 0);
  return allDonations
}
